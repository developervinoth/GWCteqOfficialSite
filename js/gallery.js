...
$.fn.ataaSlider.defaults =
  {
    showNames: true,
    height:  [
      {'minWidth':900, 'value': 400},
      {'minWidth':600, 'value': 300},
      {'minWidth':0, 'value': 200},
    ] ,
    controlHeight: '100',
    speed:500,
    openImage: true
...
function getHeight(){
  let sliderHeight = ataaSlider.width();
  for (i = 0; i < options.height.length; i++) {
    if (sliderHeight > options.height[i].minWidth) {
      return options.height[i].value;
    }
  }
}
...
function sizing(){
  ...
  ataaSlider.find('#images-holder').css('height', getHeight());
