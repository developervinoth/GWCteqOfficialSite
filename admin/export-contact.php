...
$db = getDbInstance();
$select = array('id', 'name','email','subject','message');


$chunk_size = 100;
$offset = 0;

$data = $db->withTotalCount()->get('contact');
$total_count = $db->totalCount;

$handle = fopen('php://memory', 'w');

fputcsv($handle,$select);
$filename = 'export_contact.csv';
...
for ($i=0; $i<$num_queries; $i++){
    ...
    $rows = $db->get('contact',Array($offset,$chunk_size), $select);
